...
CREATE OR REPLACE VIEW bookview AS SELECT b.id, b.publishingdate, b.title, b.version, group_concat(a.name) AS authors FROM book b JOIN ( SELECT a_1.firstname || a_1.lastname AS name, b_1.id AS bookid FROM book b_1 JOIN bookauthor ba ON b_1.id = ba.bookid JOIN author a_1 ON a_1.id = ba.authorid) a ON a.bookid = b.id GROUP BY b.id;